...
Depending on the language and platform, this ranges from gathering information when a program crashes, to inserting \gls{breakpoints} -- points in the code where the debugged program will halt, allowing the contents of its memory to be inspected, and potentially modified.
...
While \gls{gdb} supports both Java and Eclipse, the Eclipse-integration is mainly to enable the use of eclipse as an environment for writing programs with C and C++.
