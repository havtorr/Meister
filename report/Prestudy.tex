...
Download links for the tools are provided in their respective glossary entries.
~\\
...
\Gls{codecanvas} \cite{Deline2010} uses an interesting way of visualizing an entire project, everything from source-code to design documents and diagrams are layered onto a large canvas, allowing easy navigation between various elements, but is restricted to Microsoft Visual Studio, and the languages it supports.
~\\
...
The \gls{tod} \cite{Pothier2007}, as it name suggests, uses execution traces to enable its debugging and querying features.
It offers a high performance, being able to maintain usable interaction while debugging complex software, but its only visual representation of programs is the 'mural', a graph that shows event density over time.
~\\
...
%brukt i tdt4100 med blandet hell, mye unødvendig info
...
\Gls{javavis} \cite{Oechsle2002} provides visualizations in the form of \gls{uml} sequence- and object-diagrams, but does not provide any debugging features.
~\\
