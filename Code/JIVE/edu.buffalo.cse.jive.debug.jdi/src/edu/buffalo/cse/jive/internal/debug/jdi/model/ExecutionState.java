...
final class ExecutionState implements IExecutionState
{
  ...
  private final ConcurrentMap<Long, IObjectContour> oidToContour;
  ...
  private final ConcurrentMap<Long, ILineValue> threadToLine;
  private final ConcurrentMap<Long, IMethodTerminatorEvent> threadToPendingMethodReturned;
  ...
  private final ConcurrentMap<IContourMember, Object> variableToValue;
  private final Set<IMethodContour> visitedArguments;
  private final Object[] NULL_HOLDER = new Object[]
  { null };
  ...
  ExecutionState(final IExecutionModel executionModel)
  {
    ...
    this.inModelFrames = Tools.newHashSet();
    this.oidToContour = Tools.newConcurrentHashMap();
    this.threadToPendingMethodReturned = Tools.newConcurrentHashMap();
    this.keyToContour = Tools.newConcurrentHashMap(ExecutionState.IDS_INITIAL_CAPACITY);
    this.accessorKeyToLine = Tools.newConcurrentHashMap();
    this.threadToException = Tools.newConcurrentHashMap();
    this.threadToLine = Tools.newConcurrentHashMap();
    this.threadToFrames = Tools.newConcurrentHashMap();
    this.threadToInModelFrames = Tools.newConcurrentHashMap();
    this.threadSummaries = Tools.newConcurrentHashMap();
    this.variableToValue = Tools.newConcurrentHashMap();
    this.visitedArguments = Tools.newHashSet();
  }
  ...
  @Override
  public boolean isArrayCellUpdateLine(final IContourMember var, final ThreadReference thread)
  {
  ...
  @Override
  public boolean isArrayCellUpdateLine(final IMethodContour method, final IContourMember field,
