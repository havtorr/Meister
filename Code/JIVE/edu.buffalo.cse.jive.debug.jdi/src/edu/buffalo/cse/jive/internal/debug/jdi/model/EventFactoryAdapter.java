...
final class EventFactoryAdapter
{
  ...
  public static final int SMALL_ARRAY_SIZE = 20;
  ...
  public IJiveEvent createDestroyEvent(final ThreadReference thread, final long oid)
  {
    ...
    return eventFactory().createDestroyEvent(threadId, modelFactory().createUnavailableLine(),
        contour);
  }
  ...
  private IValue createCaller(final ThreadReference thread)
  {
    ...
    {
      return modelFactory().createSystemCaller();
    }
    ...
    if (executionState().containsInModelFrame(frame))
    {
      ...
      return modelFactory().createReference(executionState().lookupContour(frame));
    }
    ...
    else
    {
      ...
      if (top != null)
      {
        ...
        return modelFactory().createOutOfModelMethodReference(description, method);
      }
      ...
      return modelFactory().createOutOfModelMethodKeyReference(description, methodKey);
    }
  }
  ...
  private IValue createSyntheticTarget(final StackFrame frame)
  {
    ...
    return modelFactory().createReference(method);
  }
      ...
      final boolean createLocals)
  {
    ...
    else
    {
      ...
      return modelFactory().createOutOfModelMethodKeyReference(createDescription(f), methodKey);
    }
  }
  ...
  private IValue createThrower(final StackFrame frame)
  {
    ...
    return modelFactory().createOutOfModelMethodKeyReference(createDescription(frame), methodKey);
  }
      ...
      final String typeName)
  {
    if (object.type() instanceof ClassType)
    {
      ...
      if (summary.isResolved())
      {
        ...
        {
          return modelFactory().createResolvedValue(value, "");
        }
        // declared and actual types do not match
        return modelFactory().createResolvedValue(value, summary.actualTypeName);
      }
    }
    ...
    return modelFactory().createOutOfModelValue(object.toString());
  }
  ...
  private IValueFactory modelFactory()
  {
  ...
  private ILineValue resolveLine(final Location location)
  {
    ...
    return modelFactory().createUnavailableLine();
  }
  ...
  private IValue resolvePrimitive(final com.sun.jdi.Value value)
  {
    if (value == null)
    {
      return modelFactory().createNullValue();
    }
    ...
    if (t instanceof PrimitiveType)
    {
      ...
      {
        return modelFactory().createPrimitiveValue(escapeStringValue(value.toString()));
      }
      return modelFactory().createPrimitiveValue(value.toString());
    }
    return null;
  }
      ...
      final String typeName)
  {
    ...
    if (contourFactory().lookupInstanceContour(type.name(), object.uniqueID()) != null)
    {
      final IContextContour instance = contourFactory().retrieveInstanceContour(type.name(),
          object.uniqueID());
      // in-model value
      return modelFactory().createReference(instance);
    }
    // out-of-model value
  ...
  private IValue resolveReturnValue(final MethodExitEvent event)
  {
    ...
    {
      return modelFactory().createUninitializedValue();
    }
  }

  private IThreadValue resolveThread(final LocatableEvent object)
  {
    return modelFactory().createThread(object.thread().uniqueID(), object.thread().name());
  }
  ...
  {
    return modelFactory().createThread(object.uniqueID(), object.name());
  }
      ...
      final ObjectReference object, final Field field, final Value valueToBe)
  {
    ...
    return eventFactory().createFieldWriteEvent(threadId, modelFactory().createUnavailableLine(),
  ...
  IMethodExitEvent createSyntheticMethodExitEvent(final StackFrame frame)
  {
    ...
    return (IMethodExitEvent) eventFactory().createMethodExitEvent(threadId,
        modelFactory().createUnavailableLine());
  }
  ...
  IJiveEvent createSyntheticObjectLoadEvent(final ThreadReference thread)
  {
    ...
    return eventFactory().createTypeLoadEvent(threadId, modelFactory().createUnavailableLine(),
        object);
  }
  ...
  IJiveEvent createThrowEvent(final ExceptionEvent event, final boolean framePopped)
  {
    ...
    if (nextLine != modelFactory().createUnavailableLine())
    {
