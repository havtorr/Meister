...
class JDIEventHandlerDelegate
{
  ...
  JDIEventHandlerDelegate(final IJiveDebugTarget owner)
  {
    ...
    this.pendingCellWrites = Tools.newHashSet();
  }
  ...
  private boolean handlePendingReturned(final ThreadReference thread)
  {
    if (executionState().hasPendingMethodReturned(thread.uniqueID()))
    {
      ...
      throws AbsentInformationException
  {
    ...
    for (final LocalVariable var : vars.keySet())
    {
      ...
      if (var.isArgument())
      {
        ...
        if (executionState().observedVariable(varInstance, val, frame.thread()))
        {
          /**
      ...
      final ArrayReference arrayRef)
  {
    ...
    for (int i = 0; values != null && i < values.size(); i++)
    {
      ...
      {
        handleNewArray((ArrayReference) cellValue, location, frame);
      }
      // true if the variable was newly observed or its value changed
      if (executionState().observedVariable(cell, cellValue, frame.thread()))
      {
      ...
      throws AbsentInformationException
  {
    ...
    for (final Object o : getVariables(m))
    {
      ...
      else if (var.isVisible(frame))
      {
        final Value val = frame.getValue(var);
        // true if the variable was newly observed or its value changed
        if (executionState().observedVariable(varInstance, val, thread))
        {
          if (val instanceof ArrayReference)
          {
            ...
            if (!handleNewArray((ArrayReference) val, event != null ? event.location() : loc, frame))
            {
              ...
              if (executionState().isArrayCellUpdateLine(varInstance, thread))
              {
    ...
    if (!pendingCellWrites.isEmpty()
        && executionState().isArrayCellUpdateLine(method, null, thread))
    {
      final Set<ArrayReference> toRemove = Tools.newHashSet();
      ...
      throws IncompatibleThreadStateException, AbsentInformationException
  {
    ...
    handlePendingReturned(event.thread());
    ...
    if (event.object() != null)
    {
      ...
      if (fieldValue instanceof ArrayReference)
      {
        ...
        if (oc != null)
        {
          ...
          if (varInstance != null
              && executionState().isArrayCellUpdateLine(method, varInstance, event.thread()))
          {
      ...
      throws IncompatibleThreadStateException, AbsentInformationException
  {
    ...
    if (event.valueToBe() instanceof ArrayReference)
    {
      // dispatch a new array event and the respective cell assignments
      ...
      throws IncompatibleThreadStateException, AbsentInformationException
  {
    ...
    {
      // System.err.println("OMIT_OUT_OF_MODEL_STEP[" + frame.location() + "]");
      return;
    }
    // handle a pending returned event if necessary
    final boolean withPendingReturn = handlePendingReturned(event.thread());
