 ...
 */
final class ProgramSlice implements IProgramSlice
{
  /**
  ...
  ProgramSlice(final IAssignEvent initial)
  {
    this.chaseExceptions = Tools.newHashSet();
    this.chaseFields = Tools.newHashSet();
    this.contexts = Tools.newHashSet();
    this.enteredEvents = Tools.newHashMap();
    this.enums = Tools.newHashMap();
    this.eventSet = new BitSet();
    this.hasSnapshot = false;
    this.initial = initial;
    this.members = Tools.newHashSet();
    this.methods = Tools.newHashSet();
    this.pendingCompleted = Tools.newHashMap();
    this.pendingOutOfModel = Tools.newHashMap();
    this.returnedEvents = Tools.newHashMap();
    this.slices = Tools.newLinkedHashMap();
    this.valueChain = Tools.newHashMap();
  }
  ...
  public void chainValueDef(final IAssignEvent event)
  {
    ...
    if (valueChain.containsKey(member))
    {
      ...
      if (value != null)
      {
        // associate the original assignment with this assignment (old value)
  ...
  {
    final List<IJiveEvent> result = Tools.newArrayList(eventSet.cardinality());
    for (int i = eventSet.nextSetBit(0); i >= 0; i = eventSet.nextSetBit(i + 1))
    {
      /**
       * DO NOT lookup-- get the actual event!
       */
      final IJiveEvent e = ((ExecutionModel) initial.model()).store().getEvent(i);
      result.add(e);
    }
    return result;
  }
  ...
  {
    return (ExecutionModel) initial.model();
  }
  ...
  private void handleStructural(final IJiveEvent event)
  {
        ...
        && contexts.contains(((INewObjectEvent) event).newContour()))
    {
      ...
      final IJiveEvent de = model().store().getDestroyEvent(oid);
  ...
  void handleEnumDeclaration(final ITypeNode enumNode)
  {
    ...
    {
      final Set<IDataNode> constants = Tools.newHashSet();
