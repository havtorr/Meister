 ...
 */
class ContourFactory implements IContourFactory
{
  private static ConcurrentMap<IExecutionModel, ContourFactory> factories = Tools
      .newConcurrentHashMap();

  static ContourFactory getDefault(final ExecutionModel model)
  {
  ...
  private ContourFactory(final ExecutionModel model)
  {
    this.model = model;
  }
  ...
  IObjectContour createArrayContour(final ITypeNode concreteNode, final long oid, final int length)
  {
  ...
  IObjectContour createInstanceContour(final ITypeNode concreteNode, final long oid)
  {
    // cannot create a contour with a null schema
    if (concreteNode == null)
    {
  ...
  IContextContour createStaticContour(final ITypeNode node)
  {
    // cannot create a contour with a null schema
    if (node == null)
    {
  ...
  abstract class AbstractContour implements IContour
  {
    ...
    public Collection<IContourMember> members()
    {
      ...
      final List<IContourMember> filtered = Tools.newArrayList();
    ...
    {
      final Map<Integer, IContourMember> members = Tools.newLinkedHashMap();
   ...
   */
  abstract class ContextContour extends AbstractContour implements IContextContour
  {
        ...
        final boolean isStatic)
    {
      ...
      this.initiators = Tools.newArrayList();
    }
    ...
    public List<IMethodCallEvent> nestedInitiators()
    {
      ...
      final List<IMethodCallEvent> virtualInitiators = Tools.newArrayList();
    ...
    void addInitiator(final IMethodCallEvent initiator)
    {
   ...
   */
  interface IMutableContourMember extends IContourMember
  {
    /**
