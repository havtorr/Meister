...
final class EventFactory implements IEventFactory
{
  ...
  EventFactory(final ExecutionModel executionModel)
  {
  ...
  {
    return new RTSystemExitEvent(timestamp,
        ((ValueFactory) model.valueFactory()).createSystemThread());
  }

  @Override
  public IJiveEvent createRTSystemStartEvent(final long timestamp)
  {
    return new RTSystemStartEvent(timestamp,
        ((ValueFactory) model.valueFactory()).createSystemThread());
  }
  ...
  {
    return new SystemExitEvent(((ValueFactory) model.valueFactory()).createSystemThread());
  }
  ...
  IJiveEvent createSystemStartEvent()
  {
    return new SystemStartEvent(((ValueFactory) model.valueFactory()).createSystemThread());
  }

  IJiveEvent createThreadStartEvent(final IThreadValue thread)
  {
  ...
  private class MethodCallEvent extends InitiatorEvent implements IMethodCallEvent
  {
    ...
    @Override
    protected void addInitiator(final IInitiatorEvent initiator)
    {
  ...
  private class SystemStartEvent extends InitiatorEvent implements ISystemStartEvent
  {
    ...
    @Override
    void setTerminator(final ITerminatorEvent terminator)
    {
  ...
  private final class VarAssignEvent extends LocalDataEvent implements IVarAssignEvent
  {
    ...
    // relevant only for sliced models
    @Override
    public void setLastAssignment(final IAssignEvent event)
    {
      // System.err.println("SET_LAST_ASSIGN[" + this + " = " + event + "]");
      ...
      if (changes.size() == 1 && changes.get(0) instanceof AtomicUpdate)
      {
        ...
        // System.err.println("APPLY[" + this + " = " + lastValue + "]");
      }
    }
   ...
   */
  abstract class AssignEvent extends DataEvent implements IAssignEvent
  {
    private final IValue newValue;
    // relevant only for sliced models
    private IValue lastValue;
   ...
   */
  abstract class InitiatorEvent extends JiveEvent implements IInitiatorEvent
  {
    ...
    {
      final List<IJiveEvent> result = Tools.newArrayList();
      model.readLock();
      try
      {
    ...
    private void setLastVirtualEvent(final IJiveEvent lastChild)
    {
      ...
      else
      {
        ...
        IJiveEvent currentEvent = model.store().getEvent(((JiveEvent) lastEvent).eventId);
        // search anywhere
        if (this instanceof ISystemStartEvent)
        {
              ...
              && currentEvent.eventId() == EventFactory.EVENT_NULL)
          {
            ...
            currentEvent = model.store().getEvent(((JiveEvent) currentEvent).eventId - 1);
          }
        }
     ...
     * this consistently.
     */
    void addInitiator(final IInitiatorEvent initiator)
    {
      /**
    ...
    void setLastEvent(final IJiveEvent event)
    {
     ...
     */
    @Override
    void setVirtualId(final long virtualId)
    {
      super.setVirtualId(virtualId);
      final List<IInitiatorEvent> virtualInitiators = Tools.newArrayList();
   ...
   * and line reference.
   */
  abstract class JiveEvent implements IJiveEvent
  {
    ...
    @Override
    public final String details(final OutputFormat format)
    {
      if (format == OutputFormat.OUT_XML)
      {
        return XMLTools.eventToXML(this);
      }
      if (format == OutputFormat.OUT_CSV)
      {
        return StringTools.eventToCSV(this);
      }
    ...
    void setParentInitiator(final IInitiatorEvent event)
    {
      ((InitiatorEvent) event).setHasChildren();
      this.parentExecution = event;
    }

    void setTransaction(final ITransaction transaction)
    {
