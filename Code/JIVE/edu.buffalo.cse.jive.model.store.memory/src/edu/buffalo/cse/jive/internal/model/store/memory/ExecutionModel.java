...
import edu.buffalo.cse.jive.internal.model.store.memory.ContourFactory.ContextContour;
...
public class ExecutionModel implements IExecutionModel
{
  ...
  public ExecutionModel(final boolean createKnownTypes)
  {
    ...
    this.threadStacks = Tools.newHashMap();
    this.threads = Tools.newLinkedHashMap();
  ...
  public List<IThreadStartEvent> lookupThreads()
  {
    ...
    try
    {
      final List<IThreadStartEvent> result = Tools.newArrayList();
  ...
  {
    // event batch
    final List<IJiveEvent> events = Tools.newArrayList();
    readLock();
    try
    {
  ...
  private class TraceView implements ITraceView
  {
    ...
    @Override
    public boolean exportTo(final String target)
    {
      ...
      {
        return new JDBCExporter(store).exportToJDBC(target);
      }
      final StringBuilder sb = new StringBuilder();
      readLock();
      try
      {
        final OutputFormat format = target.endsWith("xml") ? OutputFormat.OUT_XML
            : OutputFormat.OUT_CSV;
        if (format == OutputFormat.OUT_XML)
        {
          sb.append("<events>");
        }
        for (final IJiveEvent event : store.events())
        {
  ...
  public IMemoryStore memoryStore()
  {
    return null;
