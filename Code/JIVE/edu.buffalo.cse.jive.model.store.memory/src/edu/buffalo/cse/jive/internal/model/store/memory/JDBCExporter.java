...
class JDBCExporter
{
  ...
  JDBCExporter(final Store store)
  {
    this.store = store;
  }
  ...
  {
    exportEvents(conn, store.events().iterator());
  }
    ...
    try
    {
      while (iterator.hasNext())
      {
        ...
        else if (event instanceof IVarDeleteEvent)
        {
          ...
          pstmt_cmember.setLong(3, store.model().valueFactory().createUninitializedValue().id());
          pstmt_cmember.setLong(4, event.eventId());
          pstmt_cmember.addBatch();
          stmtCount++;
        }
  ...
  {
    exportFiles(conn, store.files().iterator());
    exportLines(conn, store.lines().iterator());
    exportThreads(conn, store.threads().iterator());
    exportValues(conn, store.values().iterator());
    exportASTNodeRefs(conn, store.lookupNodeRefs().iterator());
    exportASTNodes(conn, store.lookupRoot());
    exportResolvedNodes(conn, store.lookupResolvedNodes().iterator());
    exportMDGs(conn, store.lookupRoot());
  }
  ...
  boolean exportToJDBC(final String jdbcURL)
  {
    String url = jdbcURL;
    String schema;
