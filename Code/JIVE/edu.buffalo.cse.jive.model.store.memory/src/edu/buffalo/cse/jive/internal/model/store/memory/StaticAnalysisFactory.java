...
class StaticAnalysisFactory implements IStaticAnalysisFactory
{
  private final ExecutionModel model;

  StaticAnalysisFactory(final ExecutionModel model)
  {
    this.model = model;
  }
  ...
  {
    final List<IMethodNode> clinits = Tools.newArrayList();
    final List<IMethodNode> inits = Tools.newArrayList();
    final List<IMethodNode> constructors = Tools.newArrayList();
  ...
  private final class MethodDependenceGraph implements IMethodDependenceGraph
  {
    ...
    private MethodDependenceGraph(final IResolvedLine parent)
    {
      ...
      this.dependenceMap = Tools.newLinkedHashMap();
    ...
    public String toString()
    {
      return StringTools.mdgToString(this);
    }
  }
  ...
  private final class ResolvedLine implements IResolvedLine
  {
    ...
    ResolvedLine(final LineKind kind, final int lineNo, final IResolvedLine parent)
    {
      ...
      this.parent = parent;
      this.uses = Tools.newArrayList();
      this.definitions = Tools.newArrayList();
      this.propagatedDependences = Tools.newArrayList();
    }
