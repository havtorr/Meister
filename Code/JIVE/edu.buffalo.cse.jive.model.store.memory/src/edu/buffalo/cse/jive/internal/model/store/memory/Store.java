...
import edu.buffalo.cse.jive.internal.model.store.memory.ContourFactory.IMutableContourMember;
import edu.buffalo.cse.jive.internal.model.store.memory.EventFactory.AssignEvent;
import edu.buffalo.cse.jive.internal.model.store.memory.EventFactory.JiveEvent;
...
import edu.buffalo.cse.jive.model.lib.IndexedOneToIndexedMany;
import edu.buffalo.cse.jive.model.lib.OneToIndexedMany;
import edu.buffalo.cse.jive.model.lib.OneToMany;
import edu.buffalo.cse.jive.model.lib.Tools;

class Store implements IModel
{
  ...
  {
    this.model = model;
    this.contourChildToParent = Tools.newHashMap(Store.SZ_MEDIUM);
    this.contourParentToChildren = Tools.newHashMap(Store.SZ_MEDIUM);
    this.contourStore = Tools.newConcurrentHashMap(Store.SZ_MEDIUM);
    this.counters = Tools.newConcurrentHashMap(Store.SZ_SMALL);
    this.eventStore = Tools.newArrayList(); // new PagedArrayList<JiveEvent>();
    ...
    this.noderefStore = Tools.newConcurrentHashMap(Store.SZ_NODEREFS);
    ...
    this.oidToGC = Tools.newHashMap(Store.SZ_MEDIUM);
    this.resolvedNodes = Tools.newHashMap();
    this.root = null;
    this.rootFiles = Tools.newLinkedHashMap();
    this.slicedView = null;
    this.threadStore = Tools.newConcurrentHashMap(Store.SZ_SMALL);
    ...
    this.valueIndex = Tools.newConcurrentHashMap(Store.SZ_VALUES);
    this.valueStore = Tools.newConcurrentHashMap(Store.SZ_VALUES);
  }

  public IJiveEvent getDestroyEvent(final long oid)
  {
  ...
  List<IContour> contourChildren(final IContour parent)
  {
    ...
    final List<IContour> filtered = Tools.newArrayList();
    for (final IContour contour : model)
    {
      if (slice().contexts().contains(contour) || slice().methods().contains(contour))
      {
  ...
  List<? extends IJiveEvent> events()
  {
    ...
    final List<IJiveEvent> result = Tools.newArrayList(slicedView.cardinality());
  ...
  Collection<? extends IFileValue> files()
  {
   ...
   */
  IJiveEvent getEvent(final long eventId)
  {
    return eventStore.get((int) eventId - 1);
  }

  void indexInstanceContour(final IContour contour, final long oid)
  {
  ...
  void indexStaticContour(final IContour contour)
  {
    // index by name
  ...
  void indexValue(final Object key, final IValue value)
  {
  ...
  Collection<? extends ILineValue> lines()
  {
  ...
  Map<Integer, IDataNode> lookupDataMembers(final INode node)
  {
  ...
  IFileNode lookupFile(final String fileName)
  {
    return rootFiles.get(fileName);
  }

  Set<IFileNode> lookupFiles()
  {
    final Set<IFileNode> result = Tools.newHashSet();
    result.addAll(rootFiles.values());
    return result;
  }

  IFileValue lookupFileValue(final String fileName)
  {
    return fileValueStore.lookupKey(fileName.replace('\\', '/'));
  }

  IContour lookupInstanceContour(final String typeName, final long oid)
  {
    if (typeName == null)
    {
      return null;
  ...
  ILineValue lookupLineValue(final String fileName, final int lineNumber)
  {
    return fileValueStore.lookupValue(fileName, lineNumber);
  }

  Set<ITypeNodeRef> lookupMethodExceptions(final IMethodNode node)
  {
    return methodToExceptions.lookupChildren(node);
  }

  Map<Integer, IMethodNode> lookupMethodMembers(final ITypeNode node)
  {
   ...
   * currently in place.
   */
  IJiveEvent lookupNextEvent(final IJiveEvent event)
  {
    return lookupEvent(event.eventId() + 1);
  }

  INode lookupNode(final String key)
  {
    return key == null ? null : nodeToChildNodes.lookupKey(key);
    // nodeIndex.get(key);
  }

  INodeRef lookupNodeRef(final String key)
  {
    return key == null ? null : noderefStore.get(key);
  }

  Collection<? extends INodeRef> lookupNodeRefs()
  {
    return noderefStore.values();
  }

  Set<ITypeNode> lookupNodeTypes(final INode node)
  {
   ...
   * currently in place.
   */
  IJiveEvent lookupPriorEvent(final IJiveEvent event)
  {
    return lookupEvent(event.eventId() - 1);
  }

  Collection<? extends IResolvedNode> lookupResolvedNodes()
  {
    return resolvedNodes.values();
  }

  IRootNode lookupRoot()
  {
    return root;
  }

  IContour lookupStaticContour(final String typeName)
  {
    if (typeName == null)
    {
      return null;
  ...
  Set<ITypeNodeRef> lookupSuperInterfaces(final ITypeNode node)
  {
    return typeToSuperInterfaces.lookupChildren(node);
  }

  IThreadValue lookupThread(final Long key)
  {
    return threadStore.get(key);
  }

  Set<ITypeNode> lookupTypeMembers(final INode node)
  {
    return nodeToTypes.lookupChildren(node);
  }

  IValue lookupValue(final Object key)
  {
   ...
   */
  long nextCount(final Object key)
  {
  ...
  long storeFileNode(final IFileNode node)
  {
  ...
  void storeFileValue(final String fileName, final IFileValue file)
  {
    fileValueStore.putKey(fileName, file);
  }

  void storeLine(final String fileName, final ILineValue line)
  {
    fileValueStore.put(fileName, line.lineNumber(), line);
  }

  long storeMethodNode(final IMethodNode node)
  {
  ...
  long storeNodeRef(final INodeRef nodeRef)
  {
  ...
  long storeResolvedNode(final IResolvedNode node)
  {
    // the new node's identifier
  ...
  long storeRootNode(final IRootNode node)
  {
    // the new node's identifier
    final long result = nextNodeId();
  ...
  void storeThread(final Long uniqueId, final IThreadValue value)
  {
  ...
  long storeTypeNode(final ITypeNode node)
  {
    // remove this method's entry from the node reference index
    final INodeRef ref = noderefStore.remove(node.key());
    // the new node's identifier
  ...
  long storeValue(final IValue value)
  {
  ...
  Collection<? extends IThreadValue> threads()
  {
