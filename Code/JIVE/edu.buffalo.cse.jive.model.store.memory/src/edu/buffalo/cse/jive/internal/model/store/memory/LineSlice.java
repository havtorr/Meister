...
final class LineSlice
{
  ...
  /**
   * A local array variable assignment causes a field assignment event to be issued if the local
   * references a field.
   */
  ...
  {
    this.defRelevant = Tools.newArrayList();
    ...
    this.relevantToOutOfModel = Tools.newHashSet();
    this.resolutionCursor = -1;
    this.resolutionList = Tools.newArrayList();
    this.resolutionArgs = Tools.newArrayList();
    this.resolutionMap = Tools.newHashMap();
  }
  ...
  private void lineChanged(final IJiveEvent event, final IResolvedLine oldLine)
  {
    ...
    final List<IResolvedNode> defs = Tools.newArrayList();
    ...
    final List<IResolvedNode> uses = Tools.newArrayList();
  ...
  private IResolvedNode resolveNode(final IFieldAssignEvent event)
  {
    ...
    final String validFieldAssign = validateFeldAssign(key);
  ...
  private String validateFeldAssign(final IResolvedNode key)
  {
    // validate the key as a data node
    if (!(key instanceof IResolvedData))
    {
  ...
  boolean handleEvent(final IJiveEvent event)
  {
     ...
     */
    final boolean isRelevant = isUseRelevant() || isCatchRelevant || isDefRelevant || isFieldDef
        || isEntryRelevant;
    //
    ...
    {
      // System.err.println("UNKONWN_NODE[" + event + "|" + line + "]");
      //
      ...
      if (event instanceof IFieldAssignEvent)
      {
        ...
        {
          // System.err.println(String.format("Relevant unmapped line '%s' in event '%s'.", event
          // .line().toString(), event.toString()));
      ...
      else if (event instanceof IVarAssignEvent)
      {
        ...
        // might need to chain value
        parent.program().chainValueDef((IVarAssignEvent) event);
      }
      return isRelevant;
    }
    //
    ...
    {
      // System.err.println("INFERRED_ARRAY_FIELD_WRITE_AT_START_OF_METHOD[" + event + "|" + line +
      // "]");
      //
      // relevant field assignment
      //
    ...
    if (line.kind() == LK_METHOD_DECLARATION && !parent.isClinit() && !resolutionArgs.isEmpty())
    {
      // System.err.println("ARGUMENT_NODE[" + event + "|" + line + "]");
      //
    ...
    else if (event instanceof IFieldAssignEvent)
    {
      ...
      final boolean isSupuriousArrayFieldWrite = isSpuriousArrayCellWrite
          && parent.program().isRelevantContext(((IFieldAssignEvent) event).contour());
      //
      // try to resolve as field chased by the program slice
      //
      if (isSupuriousArrayFieldWrite || parent.program().hasChaseField((IFieldAssignEvent) event))
      {
        parent.program().handleFieldDef((IFieldAssignEvent) event);
        ...
        {
          final IResolvedNode node = resolveNode((IFieldAssignEvent) event);
    ...
    else if (event instanceof IVarAssignEvent)
    {
          ...
          || (line.kind() == LK_CATCH && relevantLines.get(line.lineNumber())))
      {
        parent.handleVariableDef((IVarAssignEvent) event);
        isDefRelevant = true;
        resolveNode((IVarAssignEvent) event);
        // handle implicit use of the def variable
        if (isSelfWrite)
        {
          parent.handleVariableUse(member);
