...
public class OneToIndexedMany<K, J, V> extends ReentrantReadWriteLock
{
  ...
  public OneToIndexedMany(final int cardinality, final boolean isSortedMap)
  {
    ...
    this.delegate = Tools.newHashMap();
    this.isSortedMap = isSortedMap;
    final Map<J, V> map = Tools.newSortedMap();
  ...
  public Collection<V> allValues()
  {
    ...
    try
    {
      final List<V> result = Tools.newArrayList(delegate.size() * cardinality);
  ...
  protected Map<J, V> putIfAbsent(final K key)
  {
    ...
    if (result == null)
    {
      if (isSortedMap)
      {
        result = Tools.newSortedMap();
      }
      else
      {
        result = Tools.newHashMap();
      }
